...
export function FlyoutProvider({children}: FlyoutProviderProps) {
  ...
  return (
    <FlyoutContext.Provider value={{isVisible, toggleFlyout, showFlyout, hideFlyout}}>
      <Flyout $visible={isVisible}>
        <FlyoutMaskEscape onClick={hideFlyout}/>
        <FlyoutMenu>
          <CloseButton onClick={hideFlyout}><XIcon color="#fff" size={32}/></CloseButton>
          <MenuItem onClick={() => signOut()}><SignOutIcon color="#fff" size={32}/>Sign out</MenuItem>
...
export function useFlyout(): FlyoutContextType {
  ...
  if (context === undefined) {
    throw new Error('useFlyout must be called from a child element of FlyoutProvider')
  }
